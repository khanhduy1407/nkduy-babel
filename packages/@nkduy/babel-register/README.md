...
```sh
npm install @nkduy/babel-register --save-dev
```

## Usage

```js
require("@nkduy/babel-register");
```
...
By default `@nkduy/babel-node` and `@nkduy/babel-register` will save to a json cache in your
temporary directory.
...
```sh
BABEL_CACHE_PATH=/foo/my-cache.json @nkduy/babel-node script.js
```
...
```sh
BABEL_DISABLE_CACHE=1 @nkduy/babel-node script.js
```
